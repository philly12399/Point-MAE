...
def test(track_path='./data/output_bytrackid/car_mark_all_rotxy'):
    ...
    # a1=read_vis_points(track['000000'][1])
    # a2=read_vis_points(track['000000'][7])
    # print(DirectChamfer(a1.cuda(),a2.cuda()))
    # print(DirectChamfer(a2.cuda(),a1.cuda()))
    # print(FilterChamfer(a1,a2))
    # print(FilterChamfer(a2,a1))
    ...
    for i,tid1 in enumerate(track):
        score=[[],[]]            
        for j,tid2 in enumerate(track):
            score[0].append(FilterChamfer(read_vis_points(track[tid1][rep[i]]),read_vis_points(track[tid2][rep[j]])))
            # score[1].append(FilterHausdorf(read_vis_points(track[tid1][rep[i]]),read_vis_points(track[tid2][rep[j]])))
        cross_score[0].append(score[0])
        # cross_score[1].append(score[1])
    print(cross_score)    
    pdb.set_trace()
